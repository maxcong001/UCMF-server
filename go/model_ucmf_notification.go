/*
 * Nucmf_UECapabilityManagement
 *
 * Nucmf_UECapabilityManagement Service. © 2020, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC). All rights reserved. 
 *
 * API version: 1.0.2
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package openapi

type UcmfNotification struct {

	DicEntryId int32 `json:"dicEntryId" bson:"dicEntryId"`

	EventType EventType `json:"eventType" bson:"eventType"`

	ManAssOpRequestlist ManAssOpRequestlist `json:"manAssOpRequestlist,omitempty" bson:"manAssOpRequestlist"`

	VersionId int32 `json:"versionId,omitempty" bson:"versionId"`
}
